    ...
    <div class="display-comment mt-3 mb-3 @if (!empty($comment->parent_id)) child-comment @endif" id="comment-{{ $comment->id }}">
        <div class="comment-header">
            <div class="comment-user-data">
                <i class="material-icons">@if (!empty($comment->parent_id)) comment @else message @endif</i>
                <strong>{{ $comment->user->name }}</strong> - {{ $comment->created_at }}
            </div>
            @if (auth()->user()->isSuperAdmin() || $comment->user_id === auth()->user()->id)
                <div class="action-btn">
                    <a href="javascript:void(0)" class="btn btn-edit btn-comment p-1" data-comment-id="{{ $comment->id }}">
                    ...
                    <span href="{{ route('comments.destroy', $comment->id) }}"
                        ...
                        data-placement="top" 
                        class="btn btn-delete btn-comment p-1">
        ...
        <div id="accordion">
            <div class="card">
                <div class="p-0" id="heading{{ $comment->id }}">
                    <h5 class="mb-0">
